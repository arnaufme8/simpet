...
def make_simset_phg(
    ...
    with open(output_file, "w") as f:
        ...
        f.write(
            boolean + "simulate_PET_coincidences_only = " + sim_PET_coinc_only + "\n"
        )
        f.write(
            boolean
            + "simulate_PET_coincidences_plus_singles = "
            + sim_PET_coinc_plus_singles
            + "\n"
        )
        ...
        f.write(
            ...
            + coherent_scatter_object
            + "\n"
        )
        f.write(
            boolean
            + "model_coherent_scatter_in_tomo = "
            + coherent_scatter_detector
            + "\n"
        )
        ...
        f.write(
            ...
            + join(simset_phgdata_dir, "coh.tables")
            + '"\n'
        )
        f.write(
            string
            + 'activity_indexes = "'
            + join(simulation_dir, "rec.act_indexes")
            + '"\n'
        )
        f.write(
            string
            + 'activity_table = "'
            + join(simulation_dir, "phg_act_table")
            + '"\n'
        )
        f.write(
            string
            + 'activity_index_trans = "'
            + join(simset_phgdata_dir, "phg_act_index_trans")
            + '"\n'
        )
        f.write(
            string
            + 'activity_image = "'
            + join(simulation_dir, "rec.activity_image")
            + '"\n'
        )
        f.write(
            string
            + 'attenuation_indexes = "'
            + join(simulation_dir, "rec.att_indexes")
            + '"\n'
        )
        f.write(
            string
            + 'attenuation_table = "'
            + join(simset_phgdata_dir, "phg_att_table")
            + '"\n'
        )
        f.write(
            string
            + 'attenuation_index_trans = "'
            + join(simset_phgdata_dir, "phg_att_index_trans")
            + '"\n'
        )
        f.write(
            string
            + 'attenuation_image = "'
            + join(simulation_dir, "rec.attenuation_image")
            + '"\n'
        )
        ...
        if S == 1:
            f.write(
                ...
                + join(simulation_dir, "sampling_rec")
                + '"\n'
            )
        ...
        f.write(
            string + 'statistics_file = "' + join(simulation_dir, "rec.stat") + '"\n'
        )
        f.write(
            string
            + 'isotope_data_file = "'
            + join(simset_phgdata_dir, "isotope_positron_energy_data")
            + '"\n'
        )
        f.write(
            string
            + 'detector_params_file = "'
            + join(simulation_dir, "det.rec")
            + '"\n'
        )
        f.write(
            string + 'bin_params_file = "' + join(simulation_dir, "bin.rec") + '"\n'
        )
        ...
        if phg_hf == 1:
            f.write(
                string
                + 'history_file = "'
                + join(simulation_dir, "phg_hf.hist" + '"\n')
            )
...
def add_randoms(sim_dir, simset_dir, coincidence_window, rebin=True, log_file=False):
    ...
    with open(template, "w") as f:
        ...
        f.write(
            real + "coincidence_timing_window_in_ns = " + str(coincidence_window) + "\n"
        )
...
def simset_calcattenuation(
    simset_dir, sim_dir, output, hdr_to_copy, nrays=1, timeout=36000
):
