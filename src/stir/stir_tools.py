...
def create_stir_hs_from_detparams(scannerParams, output_file, output_format="SimSET"):
    ...
    new_file.write(
        ...
        + str(views_coordinate)
        + "] := "
        + str(scannerParams.get("num_aa_bins"))
        + "\n"
        + "matrix axis label ["
        + str(axial_coordinate)
        + "] := axial coordinate\n"
        + "!matrix size ["
        + str(axial_coordinate)
        + "] := "
        ...
        + str(scannerParams.get("num_aa_bins") * 2)
        + "\n"
        "Inner ring diameter (cm) := "
        + str(scannerParams.get("scanner_radius") * 2)
        + "\n"
        + "Average depth of interaction (cm) := "
        + str(scannerParams.get("average_doi"))
        + "\n"
...
def create_reduced_stir_hs_from_detparams(scannerParams, output_file, output_format="SimSET"):
    num_rings = scannerParams.get("num_rings")
    max_z = scannerParams.get("axial_fov") / 2
    min_z = -scannerParams.get("axial_fov") / 2
    z_crystal_size = scannerParams.get("z_crystal_size")
    ...
    new_file.write(
        ...
        + "matrix axis label [4] := segment\n"
        + "!matrix size [4] := "
        + str(2 * max_segment + 1) #THIS CHANGED TO MAX_SEGMENT
        + "\n"
        + "matrix axis label ["
        + str(views_coordinate)
        + "] := view\n"
        ...
        + str(axial_coordinate)
        + "] := "
        + matrix_size #THIS NEEDS TO CHANGE
        + "\n"
        + "matrix axis label [1] := tangential coordinate\n"
        + "!matrix size [1] := "
        + str(td_bins)
        + "\n"
        "minimum ring difference per segment := "
        + ring_difference #THIS NEEDS TO CHANGE
        + "\n"
        + "maximum ring difference per segment := "
        + ring_difference #THIS NEEDS TO CHANGE
        + "\n"
        ...
        + "\n"
        + "Number of rings := "
        + str(num_rings) #THIS NEEDS TO CHANGE??? (or maybe not)
        + "\n"
        + "Number of detectors per ring := "
        + str(scannerParams.get("num_aa_bins") * 2) #THIS NEEDS TO CHANGE??? (or maybe not)
        + "\n"
        "Inner ring diameter (cm) := "
        + str(scannerParams.get("scanner_radius") * 2)
        + "\n"
...
def add_noise(config, scannerParams, sinogram_stir, log_file):
    ...
    command = "%s -p %s %s %s %s" % (
        poison_noise,
        noisy_sinogram_stir_path,
        sinogram_stir[0:-3] + "hs",
        scannerParams.get("add_noise"),
        random.randint(1, 200000),
    )
    tools.osrun(command, log_file)

    create_stir_hs_from_detparams(
        scannerParams, sinogram_stir[0:-3] + "hs", "after_poiSson_noise"
    )
...
def FBP2D_recons(config, scannerParams, sinograms_stir, output_dir, log_file):
    ...
    new_file.write(
        ...
        + str(zoom)
        + "\n"
        + "xy output image size (in pixels) := "
        + str(xyOutputSize)
        + "\n\n"
...
def OSEM2D_recons(
    ...
    if (
        scannerParams.get("stir_scatt_corr_smoothing") == 1
    ):  # Will use smoothed SimSET scatter as additive_sinogram.
        scatt_corr_str = "additive sinogram := " + additive_sino_stir + "\n\n"
    else:
        scatt_corr_str = ""

    if (
        scannerParams.get("inter_iteration_filter") == 1
    ):  # will apply inter-iteration filter
        inter_iter_filter_str = (
            "inter-iteration filter subiteration interval:= "
            + str(scannerParams.get("subiteration_interval"))
            + "\n"
            + "inter-iteration filter type := Separable Cartesian Metz \n"
            + "  Separable Cartesian Metz Filter Parameters := \n"
            + "  x-dir filter FWHM (in mm):= "
            + str(scannerParams.get("x_dir_filter_FWHM"))
            + "\n"
            + "  y-dir filter FWHM (in mm):= "
            + str(scannerParams.get("y_dir_filter_FWHM"))
            + "\n"
            + "  z-dir filter FWHM (in mm):= "
            + str(scannerParams.get("z_dir_filter_FWHM"))
            + "\n"
    ...
    new_file.write(
        ...
        + "\n"
        + "xy output image size (in pixels) := "
        + str(xyOutputSize_aux)
        + "\n"
        "Z output image size (in pixels) := "
        + str(zOutputSize)
        + "\n\n"
        ...
        + "\n"
        + "save estimates at subiteration intervals := "
        + str(savingInterval)
        + "\n\n"
